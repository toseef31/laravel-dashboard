...
class HardyReelMediaController extends Controller
{
    ...
    {
        $request->validate([
            'reel_id' => 'required|integer|exists:hardy_reels,id',
            'media_id' => 'required|integer|exists:hardy_reel_media,id',
        ]);

        // Reset any existing thumbnail for this ephemera
        HardyReelMedia::where('reel_id', $request->reel_id)
            ->update(['thumbnail' => null]);

        // Set new thumbnail
        HardyReelMedia::where('id', $request->media_id)
