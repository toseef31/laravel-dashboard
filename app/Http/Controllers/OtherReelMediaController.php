...
use App\Models\OtherReelMedia;

class OtherReelMediaController extends Controller
{
    public function store(Request $request)
    {
        try {
            ...
            foreach ($files as $file) {
                ...
                $filePath = $file->storeAs('uploads/Other-Reel', $fileName, 'public');

                $reelMedia = new OtherReelMedia();
    ...
    public function deleteOtherReelMedia(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'id' => 'required|integer',
            ...
            $reelMedia = OtherReelMedia::find($validatedData['id']);
    ...
    {
        try {
            $reelMedia = OtherReelMedia::where('reel_id', $id)->get();
            $data = [
                'media' => $reelMedia,
                'media_path' => '/storage/'
            ];
    ...
    {
        $request->validate([
            'reel_id' => 'required|integer|exists:other_reels,id',
            'media_id' => 'required|integer|exists:other_reel_media,id',
        ]);

        // Reset any existing thumbnail for this book
        OtherReelMedia::where('reel_id', $request->reel_id)
            ->update(['thumbnail' => null]);

        // Set new thumbnail
        OtherReelMedia::where('id', $request->media_id)
