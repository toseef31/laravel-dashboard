...
class EphemeraMediaController extends Controller
{
    ...
    {
        $request->validate([
            'ephemera_id' => 'required|integer|exists:ephemeras,id',
            'media_id' => 'required|integer|exists:ephemera_media,id',
        ]);

        // Reset any existing thumbnail for this ephemera
        EphemeraMedia::where('ephemera_id', $request->ephemera_id)
            ->update(['thumbnail' => null]);

        // Set new thumbnail
        EphemeraMedia::where('id', $request->media_id)
